...
// See the Device Info tab, or Template settings
 #define BLYNK_TEMPLATE_ID    // to get Template ID Login to BlynkIot App
#define BLYNK_TEMPLATE_NAME   "Home Automation"
#define BLYNK_AUTH_TOKEN      //to get Token Login to BlynkIot App
...
{
  /*read temperature and compare with 35 and if heater is ON */
...
{
  /*read the volume and compare it with 200- inlet valve is off*/
  if((tank_volume < 2000) && (inlet_sw == 0))
  {
    enable_inlet();
    inlet_sw = 1;
    /*reflect the status of thye sw on the blynk app*/
...
void setup(void)
{
 ...
 lcd.home() ;
  ...
  Blynk.begin(BLYNK_AUTH_TOKEN);
     ...
     init_serial_tank();
     /*upadte temperature to the blynk app every one second*/ 
...
void loop(void) 
{
  ...
  /*to connect the device continously to the cloud*/
  Blynk.run();
  /*turn on the timer*/
  timer.run();
  handle_temp();
